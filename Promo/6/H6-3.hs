...
diagTree t = diagTreeH t (-1) 0

diagTreeH :: Show s => Tree s -> Int -> Int-> Diag
diagTreeH ( Node a Empty Empty) pID sID = (returnNode a sID) # (connectOutside pID sID)
diagTreeH ( Node a l Empty)     pID sID = ((returnNode a sID) === (diagTreeH l sID (sID +1))) # (connectOutside pID sID)
diagTreeH ( Node a Empty r)     pID sID = ((returnNode a sID) === (diagTreeH r sID (sID +1))) # (connectOutside pID sID)
diagTreeH ( Node a l r)         pID sID =
  ((returnNode a sID)
  ===
  ( diagTreeH l sID (sID +1) ||| diagTreeH r sID (sID +2))
  # center)
  # connectOutside pID sID


returnNode :: Show s => s -> Int -> Diag
returnNode s pID = diagNode (show s) # named pID
